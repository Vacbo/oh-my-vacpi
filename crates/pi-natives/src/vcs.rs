...
#[napi(object)]
#[derive(Default)]
pub struct VcsDiscoverOptions {
	/// Prefer Jujutsu when Git and Jujutsu share the same workspace root.
	pub prefer_jj: Option<bool>,
}
...
pub fn vcs_discover(
	env: Env,
	dir: String,
	options: Option<VcsDiscoverOptions>,
) -> Result<Option<VcsRepo>> {
	let preferred = options
		.and_then(|options| options.prefer_jj)
		.and_then(|prefer_jj| prefer_jj.then_some(core::VcsKind::Jj));
	pi_vcs::detect_with_preference(Path::new(&dir), preferred)
