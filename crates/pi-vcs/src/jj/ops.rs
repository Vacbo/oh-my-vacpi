...
impl JjWorkspace {
	...
	/// Snapshot and count the current working-copy changes relative to `@`'s
	/// merged parent tree.
	...
	/// Snapshot and render current working-copy changes in git porcelain-v1
	/// form.
...
async fn working_copy_trees(
	...
	Ok(Some((parent_tree, commit.tree(), copy_records)))
}
...
fn collect_changes(
	...
	for (path, after_value) in &after {
		...
		let source = copy_records
			...
			.or_else(|| {
				non_placeholder_copy_id(after_value).and_then(|copy_id| {
					before.iter().find_map(|(source_path, source_value)| {
						(non_placeholder_copy_id(source_value) == Some(copy_id)).then_some(source_path)
					})
				})
			});
		if let Some((source_path, source_value)) =
			source.and_then(|source_path| before.get_key_value(source_path))
		{
