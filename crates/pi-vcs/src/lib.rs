...
/// user actually works in. Colocated jj-git workspaces (same root) resolve to
/// [`Repo::Git`] so git automation remains safe.
pub fn detect(dir: &Path) -> Result<Option<Repo>> {
	detect_with_preference(dir, None)
}

/// Detect which VCS owns `dir`, using `preferred` only to break an equal-root
/// tie. A strictly deeper repository always wins.
pub fn detect_with_preference(dir: &Path, preferred: Option<VcsKind>) -> Result<Option<Repo>> {
	let jj = jj::JjWorkspace::discover(dir)?;
	let git = git::GitRepo::discover(dir)?;
	match (jj, git) {
		(None, None) => Ok(None),
		(None, Some(git)) => Ok(Some(Repo::Git(Arc::new(git)))),
		(Some(jj), None) => Ok(Some(Repo::Jj(Arc::new(jj)))),
		(Some(jj), Some(git)) => {
			let jj_wins = is_strict_descendant(jj.root(), git.root())
				|| (jj.root() == git.root() && preferred == Some(VcsKind::Jj));
			if jj_wins {
				Ok(Some(Repo::Jj(Arc::new(jj))))
			} else {
				Ok(Some(Repo::Git(Arc::new(git))))
			}
		},
	}
}
...
#[cfg(test)]
pub(crate) fn test_git_command(cwd: &Path) -> std::process::Command {
...
mod tests {
	...
	fn run_git(root: &Path, args: &[&str]) -> String {
		let output = crate::test_git_command(root).args(args).output().unwrap();
	...
	#[test]
	fn preferred_backend_breaks_only_colocated_ties() {
		let colocated = tempfile::tempdir().unwrap();
		init_git(colocated.path());
		fs::create_dir_all(colocated.path().join(".jj/repo")).unwrap();
		assert_eq!(detect(colocated.path()).unwrap().unwrap().kind(), VcsKind::Git);
		assert_eq!(
			detect_with_preference(colocated.path(), Some(VcsKind::Jj))
				.unwrap()
				.unwrap()
				.kind(),
			VcsKind::Jj,
		...
		let outer_jj = tempfile::tempdir().unwrap();
		fs::create_dir_all(outer_jj.path().join(".jj/repo")).unwrap();
		let nested_git = outer_jj.path().join("nested-git");
		fs::create_dir_all(&nested_git).unwrap();
		init_git(&nested_git);
		assert_eq!(
			detect_with_preference(&nested_git, Some(VcsKind::Jj))
				.unwrap()
				.unwrap()
				.kind(),
			VcsKind::Git,
		...
		let outer_git = tempfile::tempdir().unwrap();
		init_git(outer_git.path());
		let nested_jj = outer_git.path().join("nested-jj");
		fs::create_dir_all(nested_jj.join(".jj/repo")).unwrap();
		assert_eq!(
			detect_with_preference(&nested_jj, Some(VcsKind::Jj))
				.unwrap()
				.unwrap()
				.kind(),
			VcsKind::Jj,
