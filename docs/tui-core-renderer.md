...
Per ordinary unpinned frame: `W = max(C, L − height)`, `C' = max(C, W)`, and
the only bytes that ever touch history are the **chunk** `frame[C, C')` written
at the scrollback seam; a pinned live region clips `C'` to the exactness
boundary so mutable rows stay viewport-local. The engine also tracks
**`auditRows` (A ≤ C)** — the exact leading prefix `[0, A)`; the
committed-prefix audit (§2) samples only that prefix, so the snapshot suffix
`[A, C)` drifting never triggers a re-anchor. Scrollback therefore equals
`frame[0..C)` — every row exactly once, in order, with its content at commit
time. There is nothing to guess, nothing to defer, and nothing to reconcile:
the scroll position is irrelevant because ordinary updates never rewrite
anything a scrolled reader could be looking at.
...
1. Compose the frame (`render(width)`), collecting `liveRegionStart` and the
   optional pinning policy from the root children (absolute row indices) plus
   any `RenderStablePrefix` report.
...
3. Classify: **fullPaint** (first paint, `clearScrollback` session replace, or
   geometry change outside a multiplexer — all user gestures), the opt-in
   **divergence rebuild**, or **update**.
4. Window math as in §1. Three special rules:
   ...
   - **A pinned live region clips its offscreen mutable suffix** at the
     exactness boundary instead of snapshotting it.
...
| Emitter | Bytes | When |
|---|---|---|
| `#emitFullPaint` | home + `frame[0, C')` + window rows; with `clearScrollback`, ED3 clears history without an ED2 viewport blank | gestures or the opt-in rebuild |
...
`clearScrollback: true`. The normal callers are explicit user gestures: session
...
`Alt+L` by default). It clears native history without `ED2` first; the replay
overwrites every row from home so terminals without synchronized output do not
expose a blank viewport. A gesture pins the user to the tail, so the history
snap is acceptable.
...
- `getNativeScrollbackLiveRegionStart()` — first row that may still mutate.
  ...
  truth to re-anchor.
- `isNativeScrollbackLiveRegionPinned()` — optional policy that keeps the
  mutable suffix viewport-local rather than recording frozen snapshots as it
  scrolls off. This is for replacing dashboards, not append-shaped transcript
  content.
...
nothing, so its live-region start is the block's first row. Pinning is
propagated from the first live block; tool execution uses it for replacing
...
5. **NEVER probe the viewport position or fork on platform in the update
   ...
   reintroduce them.
6. **Mutable content stays below the exactness boundary.** Only declare rows
   exact when their bytes are stable: app-layer renderers may advance that
   boundary only for rows they explicitly declare settled; the engine trusts
   the declaration and audits exact rows after they commit. Unpinned mutable
   rows may commit as frozen snapshots, deliberately outside the exactness
   claim.
