...
async function git(cwd: string, ...args: string[]): Promise<string> {
	...
	const child = Bun.spawn(["git", ...args], { cwd, env, stderr: "pipe", stdout: "pipe" });
...
describe("VcsRepo", () => {
	...
	test("prefers Jujutsu only when requested for a colocated root", async () => {
		const root = await repository();
		await mkdir(join(root, ".jj", "repo"), { recursive: true });

		expect(vcs.repo(root)?.kind()).toBe("git");
		expect(vcs.repo(root, { preferJj: true })?.kind()).toBe("jj");
	});
