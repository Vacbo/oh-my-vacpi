...
describe("model thinking derivation", () => {
	...
	it("encodes anthropic transport mode and adaptive wire maps in metadata", () => {
		...
		expect(opus46.thinking?.efforts).toEqual([Effort.Minimal, Effort.Low, Effort.Medium, Effort.High, Effort.Max]);
		...
		// Bedrock Converse reuses the 4.6 ladder because output_config.effort does not accept "xhigh".
		expect(opus47Bedrock.thinking?.efforts).toEqual([
			Effort.Minimal,
			Effort.Low,
			Effort.Medium,
			Effort.High,
		...
		expect(opus46Bedrock.thinking?.efforts).toEqual([
			Effort.Minimal,
			Effort.Low,
			Effort.Medium,
			Effort.High,
		...
		expect(sonnet46.thinking?.efforts).toEqual([Effort.Minimal, Effort.Low, Effort.Medium, Effort.High]);
		// Fable/Mythos 5 on the Messages API ride the Opus 4.7+ ladder: xhigh plus a top "max" tier.
		expect(mythos.thinking?.efforts).toEqual([
			Effort.Minimal,
			Effort.Low,
			Effort.Medium,
			Effort.High,
		...
		// Sonnet 4.6 caps at high; neither XHigh nor Max is in the supported set.
...
describe("Effort.Max clamping and mapping invariants", () => {
	it("clamps Effort.Max down for OpenAI/Gemini/Sonnet models that do not support it", () => {
	...
	it("does not promote XHigh to Max for non-adaptive ladders that happen to expose Max", () => {
		const sparseCustom = createModel({
			id: "custom-sparse-max",
			api: "openai-responses",
			provider: "openai",
			baseUrl: "https://example.com",
			thinking: { mode: "effort", efforts: [Effort.Low, Effort.Max] },
		});
