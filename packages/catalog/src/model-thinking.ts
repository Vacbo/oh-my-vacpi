...
// Opus 4.6 adaptive thinking exposes `low`, `medium`, `high`, `max` — no in-between `xhigh` level.
// Internally we still expose Minimal/Low for symmetry with the rest of the catalog; clamping handles
// the Minimal -> low collapse at the mapper layer.
const ANTHROPIC_OPUS_46_EFFORTS: readonly Effort[] = [
	Effort.Minimal,
	Effort.Low,
	Effort.Medium,
	Effort.High,
...
/**
 * Legacy `xhigh` requests against adaptive ladders whose top tier is `max`
 * (Opus 4.6 on Messages, every adaptive Opus/Fable/Mythos on Bedrock) promote
 * to `Effort.Max`: the API's `max` slot replaces the old `xhigh` slot there.
 * A custom ladder like `[Low, Max]` must NOT promote — it clamps downward.
 */
