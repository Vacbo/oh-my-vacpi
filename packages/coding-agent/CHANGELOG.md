...
- Added `/context-export <task>`: a model-driven repository context exporter for manual upload to an external chat model (e.g. ChatGPT). The command binds the exact task, issues a workflow ID, activates the new default-inactive `context_export` built-in on demand (its schema never reaches ordinary model requests), and submits a workflow prompt as a turn in the ACTIVE session, so the selecting agent keeps the full prior conversation when choosing files. The tool runs a two-call preview/write protocol over an ordered include/exclude selection program (`base: "all"` or `"none"`; exact repo-relative paths, recursive directories, 1-indexed inclusive line slices) resolved against a gitignore-respecting, hidden-including, symlink-free native inventory. Previews render the full Markdown bundle in memory with exact-byte payload preservation (BOM/CRLF/trailing whitespace/terminal-newline metadata), collision-safe fences, a sensitive-path denylist, binary skips, configured/environment-secret detection, and a 900k o200k_base token budget; `write` atomically hard-link-publishes exactly the previewed bytes to `prompt-exports/` without ever overwriting.
...
- Added tool pinning to the Tool Control Center: `ctrl+p` on a built-in row toggles it in `tools.essentialOverride`, the skills-pinning equivalent for tools (pinned tools are always loaded at session start; unpinned built-ins load on demand when `tools.discoveryMode` hides them behind `search_tool_bm25`). An empty override means the `read`/`bash`/`edit` defaults, so the first pin materializes the defaults before applying, and a result equal to the default set normalizes back to empty ("leave empty to use defaults"). Pinned rows carry the `(pinned)` badge and the inspector shows pin state with wording per state; the `ExtensionList`/`InspectorPanel` pin affordances are now callback-driven instead of skill-hardcoded, so the extensions and tools dashboards share them.
- Added the model-facing `tui_drive` tool (discoverable): the input-injection counterpart to the read-only `tui_observe`, for testing the omp TUI itself end to end. It spawns commands in an in-repo PTY (native `PtySession` + `@xterm/headless` recorder; up to 4 concurrent sessions that persist across tool calls) with actions `start`/`input`/`wait`/`screen`/`screenshot`/`diff`/`resize`/`kill`/`list`. `input` types literal text (`"\n"` → Enter) and named keys via pi-tui's new `encodeKey`; `wait` matches a regex against the rendered screen or waits for output to go idle. Driven omp children are correlated against the live-session registry (descendant-pid walk + cwd + start-time), enabling styled mirror `screenshot`s and a cmux-free `diff` of the child's internal renderer snapshot against the PTY emulator screen. The spawn env is scrubbed of terminal-identity variables (`CMUX_*`, `KITTY_*`, `GHOSTTY_*`, `ITERM_*`, `WEZTERM_*`, `TMUX*`, `TERM_PROGRAM*`) and pinned to `TERM=xterm-256color` via the new PTY `envClear` option, so children can no longer inherit a stale cmux surface id or negotiate kitty keyboard mode. `TerminalSnapshotRecorder` gained an `onData` option (xterm stdin enabled) so the emulator's DA1/DSR query replies are forwarded back into the PTY and full-screen apps don't stall waiting on them.
- Added skill discovery (`skills.discoveryMode: "search"`): instead of every skill costing a permanent summary line in the system prompt's `<skills>` block, only skills matching the `skills.pinnedSkills` globs stay listed; the rest collapse into a one-line `<skills-discovery>` roster and join the `search_tool_bm25` BM25 corpus as `skill:`-prefixed entries. Skill matches are never "activated" — the search result carries `{name, description, read: "skill://<name>"}` and the model reads the skill directly, so there is no toolset mutation or persisted activation state. The search tool stays available even when tool discovery resolves to off, and skills never hide when the search tool is absent from the active toolset (they would be unreachable). Frontmatter-hidden skills (`hide: true` / `disable-model-invocation: true`) stay out of both the listing and the corpus.
- Added SKILL.md body indexing to skill discovery: under `skills.discoveryMode: "search"`, each corpus entry carries a capped body excerpt (2000 chars, index-only, never displayed), so paraphrase queries match skills whose trigger phrasing lives in the body rather than the one-line description (e.g. "make my writing sound less like an AI wrote it" finds `humanizer`). The body is already in memory at load time, so this costs no extra IO; entries stay deterministic and offline — no embedding provider involved.
- Added skill pinning to the Extension Control Center (`/extensions`): `ctrl+p` on a skill row toggles its literal name in `skills.pinnedSkills`, pinned skills carry a `(pinned)` badge, and the inspector pane shows pin state with provenance when the pin comes from a glob (e.g. `via cmux*`). The Settings panel keeps a "Pinned Skills" text entry (tools tab, shown only while Skill Discovery is `search`) for manual glob editing, backed by new generic array-setting support: array-typed settings with `ui` metadata render as comma-separated text inputs with round-trip parsing and array-aware change detection (previously array settings could not appear in the panel at all). The Pinned Skills input also suggests while you type: the five closest skill names from the discovered pool rank under the input (fuzzy-matched like the editor's autocomplete; glob segments preview their actual matches), ↑/↓ choose, Tab accepts and primes the next segment, and a live `matches N of M skills` footer counts what the full pattern list currently covers.
- Added inline previews to `tui_observe native_screenshot`: the result now attaches a downscaled copy of the capture (same size budget as browser screenshots), so the image renders directly in the session TUI and the user sees exactly what the model received; the summary keeps the full-resolution PNG path for `inspect_image`. Preview problems never fail a capture that already succeeded. The mirror `screenshot` action already inlined its capture.
- Added emulator-side TUI capture to `tui_observe` for sessions hosted in cmux: `emulator_screen` reads the text the terminal actually rendered (via `cmux read-screen`), and `render_diff` compares it row by row against the internal xterm-headless snapshot, localizing a rendering bug to one side (emitted wrong by the TUI vs interpreted differently by the emulator). The diff detects vertical scroll between the two non-simultaneous captures and reports it as a single `scrollOffset` instead of a wall of false mismatches; in live testing it isolated a one-frame spinner advance on an otherwise identical 57-row screen. Sessions record `CMUX_SURFACE_ID` into the live-session registry at startup (`cmuxSurfaceId`, also shown by `list`/`snapshot`), so any process can read another session's screen. Both actions are read-approval, and the tool doc gained a text-vs-pixels decision note (text grids cannot show inline images, glyph rendering, or colors).
...
- Added region selection to `tui_observe screenshot`: pass `rows` (`"N"` or `"N-M"`), optional `cols`, or a raw CSS `selector` to crop the PNG to a TUI region, or `highlight: true` to outline that region on a full-page capture instead. The capture injects a positioned overlay onto the mirror's photo page and reuses the headless browser's element-clip screenshot, and the result returns the selected `text` (rows joined, optionally column-sliced) alongside the region, so a chosen part of the UI can be attached to a prompt with its coordinates. Verified end to end against a fixture mirror: a two-row crop renders at the rows' height (vs the full page) with `selection.text` matching the rows, while `highlight` keeps full-page dimensions.
- Added a drag-select overlay to the human-viewable mirror page (`/sessions?run=<id>`): drag a box over the terminal grid and it resolves the covered `data-terminal-row` rows and intersected `data-terminal-cell` column span, draws a live selection box, and surfaces a readout with the matching `tui_observe screenshot run=… rows=… cols=…` command (one-click copy) plus the selected text, so a human can point at a region and hand its coordinates (or the cropped picture) to the agent. The overlay is injected only on the mirror page, never the `mode=photo` render, so region screenshots stay overlay-free. Verified end to end with a headless synthetic drag (rows 1-2 selection yields the right rows/cols, command, and text, excluding neighbor rows).
...
- Added `/context full` (alias `/context detailed`), an in-session context inspector that renders, byte-for-byte, exactly what the model sees this session as an expandable tree: the system prompt blocks (segmented into their sections, with per-skill and per-context-file leaves), the wire tool schemas, and every message (broken into its text, thinking, tool-call, and result parts), each drillable down to its raw content. It complements the `/context` token summary (which answers "what roughly fills the window") with "show me exactly what is in there", reuses the same token accounting so the tree's total matches the summary's used-token figure, and shows secrets raw since the user owns them locally. The tree is backed by a renderer-agnostic `ManifestNode` model (the foundation for the harness-observability read API) that also renders as an indented outline on the ACP/print path. Navigate with arrows/Enter to expand and `←` to collapse; the default view auto-expands the system prompt blocks plus the top-level categories and leaves raw leaf content collapsed until requested.

#### Changed

- Changed native terminal-window capture to resolve the target window by the session's process ancestry first (a `ps` ppid walk from the session pid to the window-owning app, picking that app's largest window), falling back to `tui.nativeCapture.preferredApp` and app-name hints. Added `cmux` to the hint list. Previously selection was app-name-only: it failed as `no-window` inside unlisted terminals and as `ambiguous-window` when one app owned auxiliary windows (e.g. cmux's Settings panel) or two sessions ran in two terminals.
...
- Fixed background-async tool boxes respraying native scrollback: every output tick of a backgrounded `bash`/`task` job arrived as a partial update and kept mutating a transcript block that had already reported finalized (the `async.state === "running"` accepted-freeze), so the engine's committed-prefix audit re-anchored and recommitted the box plus everything below it, one near-identical `[box, streaming thinking]` snapshot per tick; the background-start notice could also be replaced before its rows ever committed, vanishing from history entirely. `ToolExecutionComponent.updateResult` now drops partial updates once the block is frozen: the box stays on its stable background-start notice (live output remains reachable via `jobs://` and `job` polls), and only the real final result (completed/failed) rewrites it, the engine's accepted one-off late-result repaint. Repro harness (8 ticks, 120x12 virtual terminal): pre-fix 69 tape rows with 52 sprayed tick rows and the start notice lost; post-fix 18 rows with every block exactly once.
- Fixed idle compaction racing the plan-approval overlay: while a plan review awaits a decision the idle timer neither arms nor fires, and it re-arms when the overlay closes. The review's own options (compact context, keep context, fresh session) own the context decision, so background compaction no longer preempts them; previously, idling at the approval until idle compaction ran made a subsequent "Approve and compact context" fail with `Compaction failed: Nothing to compact (session too small)`. Compaction requests that find nothing to summarize (already compacted, or session too small) now raise the typed `NothingToCompactError` and surface as a notice instead of a failure (manual `/compact` included), and plan approval proceeds to dispatch execution either way.
- Fixed mirror pages (sessions view and `mode=photo` screenshots) leaking 1-3px horizontal bands of page background between every row of a background-tinted block (user message bubbles, tool boxes): cell backgrounds were painted by inline spans, which only cover the font's content area instead of the full line box, and the fractional `1.35em` row pitch added subpixel seams on top. Cells are now full-height inline-blocks (`vertical-align: top`, immune to Nerd Font/emoji fallback baseline drift) on an integer-pixel row grid, and the duplicated page/photo terminal CSS collapsed into one parameterized `terminalStyle` helper. Verified by pixel-profiling before/after captures of the same snapshot: tint runs went from 19-20px chunks separated by 3px page-bg gaps to one unbroken 405px run.
- Fixed box-drawing characters (table borders, tool-box frames) breaking continuity at every row boundary on the mirror pages: unlike a real terminal, which draws U+2500-257F procedurally to fill the cell, the browser uses the font's glyph, which only spans its natural line height (~1.286em in Menlo), leaving a 1px seam per row at 14px font in a 19px row. Both mirror styles now use a 15px font whose box glyphs span ~19.3px and get cropped to the row by `overflow: hidden`, fusing vertical strokes across rows (longest unbroken border stroke measured 36 → 497 device px at DPR 2, identical fusion in WebKit and Chromium; descenders unaffected). Remaining sub-pixel breaks at `├`/`┼` junctions are Menlo glyph anatomy, not row seams.
- Fixed singular counts rendering as plurals in tool previews: collapsed JSON args and task output summaries showed `[1 items]`/`{1 keys}`; they now pluralize via `pluralize()`.
- Fixed the footer todo list drawing a second `└` connector on the first task at the same column as the phase's connector, which made the task read as a sibling of its phase; tasks now indent uniformly under the phase header.
- Fixed mirror photo-page screenshots (`tui_observe screenshot`, `tui_drive screenshot`) carrying a garbled strip of wrapped screen text across the top of the PNG: `renderTerminal` emits an off-screen `.sr-text` copy of the snapshot text for DOM consumers, but the photo stylesheet (unlike the main sessions page) shipped no `.sr-text` rule, so photo mode painted the full screen text as a visible block above the terminal grid and `fullPage` captures included it. The photo and sessions stylesheets now share the hiding rule, and the terminal font stack gained per-glyph Nerd Font fallbacks (Symbols/JetBrainsMono/FiraCode/Hack/MesloLGS NF) so status-line icons render as real glyphs instead of tofu boxes when a patched font is installed; regression test asserts the photo page serves the text layer hidden.
- Fixed `native_screenshot` window enumeration on macOS 26: the CFArrayRef returned by `CGWindowListCopyWindowInfo` no longer toll-free bridges to a JS array in JXA, so `ObjC.deepUnwrap` produced an opaque proxy, the enumeration script threw, and every capture failed as `no-window`. The script now re-tags the ref via `ObjC.castRefToObject` (keeping the direct path for older macOS) and reports window bounds for main-window selection.
...
- Fixed BM25-activated built-in tools (e.g. `todo_write` under `tools.discoveryMode: "all"`) vanishing across session resume, in-process session switch, branch, and history rewind: the `mcp_tool_selection` entry now persists the union of MCP and built-in discovery selections (names cannot collide; MCP tools carry the `mcp__` prefix), the sdk restores non-MCP names through the discovery filter's `restored` set, and `#restoreSelectionsForSessionContext` reseeds built-in selections on every in-process context restore. Previously a resumed session re-hid the tool while the conversation history still referenced it, so the model's next call failed with "Tool todo_write not found" until it re-discovered the tool via `search_tool_bm25`.
- Fixed the eager-todo forceActive guard in session assembly checking upstream's renamed tool (`todo`) instead of the fork's `todo_write`, which made `todo.eager` + `tools.discoveryMode: "all"` hide the todo tool at session start and silently skip the eager prelude. The computation now lives in `computeForceActiveToolNames()` with a regression test asserting every forced name resolves to a registered built-in.
- Fixed `eager-todo` system reminder occasionally producing `todo_write({})` on the first turn of new sessions under Anthropic's `tool_choice: { type: "tool", name }` forcing. The reminder was prose-only and left the model with no literal JSON anchor; under named-tool forcing Anthropic can emit a `tool_use` block with empty input, which `parseStreamingJson` coerces to `{}` and the todo schema then rejects. The reminder now inlines a literal top-level `{"op":"init","list":[...]}` example (with `op`/`list` as the top-level keys, never wrapped in an `ops` array) so the forced emission has a copy-pasteable shape, with a regression test that parses the inlined sample and asserts the flat single-op shape, never the legacy `ops` wrapper.
...
- Fixed two more `eager-todo` reminder defects: it referenced a nonexistent `details` metadata field (the reminder now states the `init` op only accepts phase names and task-label strings, so no task metadata fields are invented) and it now warns against wrapping the top-level `op`/`list` keys in an `ops` array.
