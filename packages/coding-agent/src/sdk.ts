...
async function createAgentSessionScoped(options: CreateAgentSessionOptions): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		const toolSession: ToolSession = {
			...
			// Live getter, not a bound closure: tools are built before any host mode
			// exists, so the property stays absent until an interactive host registers
			// its restart rail (and disappears again if it clears it). Print, ACP,
			// SDK-without-host, and subagent sessions therefore never claim restart.
			get requestRestart() {
		...
		// `ssh` is discovered from host config rather than listed in `BUILTIN_TOOLS`, so
		// `createTools`' allowlist filter never sees it. Mirror that filter here (an empty
		// list means "unset", exactly as `createTools` reads it) or an explicit `toolNames`
		// without `ssh` would still register the device and advertise it in the xd:// catalog.
		const sshRequested =
			options.toolNames === undefined ||
			options.toolNames.length === 0 ||
			normalizeToolNames(options.toolNames).includes("ssh");
		if (!restrictToolNames && sshRequested) {
			const sshTool = await logger.time("createTools:ssh:session", loadSshTool, toolSession);
			if (sshTool) {
				const wrapped = wrapToolWithMetaNotice(sshTool);
				toolRegistry.set(sshTool.name, wrapped);
				toolSession.xdev?.builtInNames.add(sshTool.name);
			}
		}
		...
		if (deferMCPDiscoveryForUI) {
			for (const name of collectPendingMCPToolNames(options.toolNames)) {
				if (!toolRegistry.has(name)) {
					toolRegistry.set(
						name,
						new ExtensionToolWrapper(wrapToolWithMetaNotice(createPendingMCPTool(name)), extensionRunner) as Tool,
					);
				}
		...
		// `/ssh add` and `/ssh remove` rewrite the host config while this session is
		// live, so `AgentSession.refreshSshTool()` rebuilds the device through here.
		// Read the cwd at call time: a session that moved projects must discover the
		// hosts of the project it is in now. `null` means "no host left".
		const reloadSshTool = async (): Promise<AgentTool | null> => {
			...
			// Same wrap order as the registry: meta-notice spill first, approval gate outermost.
			return new ExtensionToolWrapper(wrapToolWithMetaNotice(sshTool), extensionRunner) as AgentTool;
		};
		...
		session = new AgentSession({
			...
			reloadSshTool,
			requestedToolNames: explicitlyRequestedToolNameSet,
		...
		if (mcpManager && !options.mcpManager) {
			...
			// session — `setOnToolsChanged` only fires on the *next* change. Deferred
			// UI discovery starts with pending placeholders; refreshing with the
			// manager's still-empty tool list here would wipe them before the caller
			// can use the session.
			if (!deferMCPDiscoveryForUI) {
