 ...
 * sections silently no-op when their target tools are inactive.
 *
 * Centralized so names stay in lockstep with the `BUILTIN_TOOLS` registry:
 * this fork's todo tool is `todo_write` (upstream renamed theirs to `todo`).
 */
export function computeForceActiveToolNames(settings: Settings, hasTool: (name: string) => boolean): Set<string> {
	...
	if (settings.get("todo.eager") !== "default" && settings.get("todo.enabled") && hasTool("todo_write")) {
		forceActive.add("todo_write");
	}
	if (settings.get("task.eager") !== "default" && hasTool("task")) {
