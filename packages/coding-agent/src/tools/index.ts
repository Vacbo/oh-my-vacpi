...
export function createGrepTool(cwd: string): Tool {
	return new GrepTool(createLegacyToolSession(cwd));
}

export function createFindTool(cwd: string): Tool {
	return new GlobTool(createLegacyToolSession(cwd));
}
...
export const BUILTIN_TOOLS: Record<BuiltinToolName, ToolFactory> = {
	...
	search_skills: SearchSkillsTool.createIf,
};
...
export async function createTools(session: ToolSession, toolNames?: string[]): Promise<Tool[]> {
	...
	const isToolAllowed = (name: string) => {
		...
		// Skill-only semantic search. BM25 tool/MCP activation is gone — `xd://`
		// devices replaced it — so the tool exists purely for skill lookup and
		// loads only under `skills.discoveryMode: "search"`.
		if (name === "search_skills") return session.settings.get("skills.discoveryMode") === "search";
	...
	// out of ordinary model requests. Restricted sessions are skipped outright —
	// a structured child must expose exactly the names its host granted.
	...
	const registryOnlyNames = new Set(registryOnlyEntries.map(([name]) => name));
	...
	if (xdevEnabled) {
		...
		for (const tool of tools) {
			const mountable =
				mountBuiltinTools &&
				!registryOnlyNames.has(tool.name) &&
				isMountableUnderXdev(tool) &&
				tool.name in BUILTIN_TOOLS;
	...
	if (xdevEnabled) {
		// Registry-only built-ins stay registered but never join the active set:
		// an explicit tool request keeps them top-level (nothing to mount), so
		// they would otherwise leak into the request's schema here.
		const finalActiveNames = new Set(tools.map(tool => tool.name).filter(name => !registryOnlyNames.has(name)));
