...
/**
 * Whether a recorded tool name is a todo result in a persisted transcript.
 *
 * `todo_write` is the pre-v17 spelling: sessions written before the rename
 * still carry it, so replay and rendering MUST accept both names. Live code
 * (registration, active-tool checks, emitted reminders, tool_choice) compares
 * against the canonical `todo` only.
 */
export function isHistoricalTodoToolName(toolName: string | undefined): boolean {
	return toolName === "todo" || toolName === "todo_write";
}
...
export function getLatestTodoPhasesFromEntries(entries: SessionEntry[]): TodoPhase[] {
	for (let i = entries.length - 1; i >= 0; i--) {
		...
		if (message.role !== "toolResult" || !isHistoricalTodoToolName(message.toolName) || message.isError) continue;
