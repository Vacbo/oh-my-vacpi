...
export function formatConflictWarning(
	...
	out.push(
		'Per-id bulk: `write({ path: "conflict://*", content: "1: @ours\\n2: @theirs\\n…" })` (or `<file>:conflict://*` to guard the target file) resolves each listed id with that side in ONE call — the cheapest way through many pick-one conflicts; unlisted ids stay registered.',
	);
	out.push(
