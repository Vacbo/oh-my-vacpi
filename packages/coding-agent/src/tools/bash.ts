...
import type { TerminalSnapshot } from "../session/terminal-snapshot";
...
/** `terminalSnapshot` is the only field unique to the interactive (PTY) result
 *  variant, so a present snapshot both identifies that variant and is the exact
 *  payload the caller records — narrow straight to its non-optional form. */
function hasTerminalSnapshot(
	result: BashResult | BashInteractiveResult,
): result is BashInteractiveResult & { terminalSnapshot: TerminalSnapshot } {
	return "terminalSnapshot" in result && result.terminalSnapshot !== undefined;
}
