...
export class HttpTransport implements MCPTransport {
	...
	async #executeRequest<T>(
		...
		try {
			...
			// A rejection while `stage` is still "send" came out of `fetch` itself,
			// which names no endpoint (Bun reports a bare "Unable to connect").
			// Wrap it with the URL we tried before typing it, so logs and `/mcp`
			// say which endpoint failed; `normalizeMCPTransportError` recovers the
			// original `code` from the `cause` chain, so classification is unchanged.
			const failure =
				stage === "send" && error instanceof Error && !(error instanceof MCPTransportError)
					? new Error(`fetch ${this.config.url} failed: ${error.message}`, { cause: error })
					: error;
			throw normalizeMCPTransportError(failure, { transport: "http", stage, traceId });
		} finally {
			operation.clear();
		}
	}
