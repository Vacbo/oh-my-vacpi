...
import { getLatestUpstreamRelease } from "./cli/update-cli";
...
/**
 * Newest upstream version available to merge, or undefined when upstream is not
 * ahead of this build. Informational only: this fork merges upstream source via
 * `omp update`'s agent session and never installs an upstream artifact, so the
 * answer is reported, never acted on.
 */
async function checkForUpstreamMergeTarget(currentVersion: string): Promise<string | undefined> {
	if (!settings.get("startup.checkUpdate")) {
		return;
	}
	try {
		const upstreamVersion = await getLatestUpstreamRelease({ timeoutMs: 5_000 });
		return Bun.semver.order(upstreamVersion, currentVersion) > 0 ? upstreamVersion : undefined;
	} catch {
		return undefined;
	}
}
...
async function runInteractiveMode(
	...
	upstreamMergeTargetPromise: Promise<string | undefined>,
	...
	const checkedUpstreamMergeTarget = upstreamMergeTargetPromise.catch(() => undefined);
	...
	// A resolved upstream check must not insert its banner into a partial transcript.
	checkedUpstreamMergeTarget.then(upstreamVersion => {
		if (!settings.get("startup.checkUpdate")) {
			return;
		}
		if (upstreamVersion) {
			mode.showUpstreamMergeAvailableNotification(upstreamVersion);
		}
	});
...
export async function runRootCommand(
	...
	if (mode === "acp") {
		...
		if (mode === "rpc" || mode === "rpc-ui") {
			...
			const upstreamMergeTargetPromise = checkForUpstreamMergeTarget(VERSION).catch(() => undefined);
