 ...
 *   `--resume=<target>` forms) — the relaunch resumes `resumeTarget` instead;
 ...
 * `resumeTarget` is the exact durable session path and is omitted for a session
 * that never materialized on disk; the relaunch then starts fresh with the same
 * configuration.
 */
export function restartArgv(argv: string[], resumeTarget: string | undefined): string[] {
	const kept: string[] = [];
	...
	if (resumeTarget !== undefined) kept.push("--resume", resumeTarget);
	return kept;
}
