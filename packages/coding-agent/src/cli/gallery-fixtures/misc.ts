...
export const miscFixtures: Record<string, GalleryFixture> = {
	...
	// Synthetic extension tool with no dedicated renderer — exercises the generic
	// fallback (`#formatToolExecution`) path so its padded, state-tinted block is QA'd.
	custom_report: {
		label: "Custom Report",
		streamingArgs: { subject: "lsp" },
		args: {
			subject: "lsp",
