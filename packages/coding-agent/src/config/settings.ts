...
import { LEGACY_FIREPASS_SELECTOR, migrateFirePassSelectorValue } from "./firepass-selector";
import {
...
/**
 * v17 built-in tool renames. `todo_write` became `todo`, and the BM25 tool
 * `search_tool_bm25` was replaced by the skill-only `search_skills`. Neither old
 * id survives as a callable alias, so a settings array still holding one would
 * silently point at a tool that no longer exists — a stale `tools.disabledTools`
 * entry would quietly stop disabling anything.
 */
const RENAMED_TOOL_IDS: Record<string, string> = {
	todo_write: "todo",
	search_tool_bm25: "search_skills",
};

/**
 * Settings whose array entries are built-in tool ids, as `[root, leaf]`.
 * `tools.disabledTools` is the persisted /tools off-switch; `tools.xdevInlineDevices`
 * matches `xd://` device names, which are tool names. Skill and server lists are
 * deliberately absent: a skill named `todo_write` is a different namespace.
 */
const TOOL_ID_ARRAY_SETTINGS: ReadonlyArray<readonly [root: string, leaf: string]> = [
	["tools", "disabledTools"],
	["tools", "xdevInlineDevices"],
];

/**
 * Canonicalize one tool-id array, or return undefined when it is already
 * canonical (so the migration stays idempotent and rewrites nothing it need
 * not). Renamed ids are replaced, first-appearance order is preserved, and a
 * duplicate the rewrite creates — a config listing both `todo_write` and `todo`
 * — collapses to one entry. Non-string entries pass through untouched; dropping
 * those belongs to the array reader, not to a rename migration.
 */
function canonicalizeToolIdArray(value: unknown): unknown[] | undefined {
	...
	for (const entry of value) {
		...
		const canonical = RENAMED_TOOL_IDS[entry] ?? entry;
...
/** Rewrite renamed tool ids in both the nested (`tools: { disabledTools }`) and flat (`"tools.disabledTools"`) shapes. */
function migrateRenamedToolIds(raw: RawSettings): void {
	for (const [root, leaf] of TOOL_ID_ARRAY_SETTINGS) {
		const rootValue = raw[root];
		if (isRecord(rootValue)) {
			const nested = canonicalizeToolIdArray(rootValue[leaf]);
			if (nested) rootValue[leaf] = nested;
		}

		const flatKey = `${root}.${leaf}`;
		const flatValue = raw[flatKey];
		if (Array.isArray(flatValue)) {
			const target = isRecord(raw[root]) ? (raw[root] as Record<string, unknown>) : {};
			if (!Array.isArray(target[leaf])) {
				target[leaf] = canonicalizeToolIdArray(flatValue) ?? flatValue;
				raw[root] = target;
			}
			delete raw[flatKey];
		}
...
export class Settings {
	...
	async #loadGlobalSettings(): Promise<void> {
		...
		// The raw migration canonicalized legacy Fire Pass selectors in memory;
		// write the canonical form back so external readers and future launches see
		// it too. Only the global config is rewritten here: it is this instance's
		// own writable store, whereas overlays and capability-sourced settings are
		// read-only inputs we must never author. The on-disk text check keeps the
		// write to files that genuinely still hold the legacy selector.
		if (this.#persist && this.#configPath) {
			try {
				const configFile = Bun.file(this.#configPath);
				if ((await configFile.exists()) && (await configFile.text()).includes(LEGACY_FIREPASS_SELECTOR)) {
					await this.#writeYamlAtomically(this.#configPath, this.#global);
				}
			} catch (error) {
				logger.debug("Settings: could not persist Fire Pass selector migration", { error });
			}
		}
	}
	...
	async #readProjectSettings(quarantineInvalid: boolean): Promise<ProjectSettingsReadResult> {
		...
		const nativeProject = quarantineInvalid
			...
			: (this.#unwrapYamlLoadResult(projectConfigPath, await this.#loadYamlIfPresent(projectConfigPath, false)) ??
				{});
		// The load above canonicalized any legacy Fire Pass selector in memory; persist
		// it for the native project config, whose file this instance owns. Only the
		// authoritative load rewrites the file: `quarantineInvalid` marks the path that
		// already owns on-disk repair, while `reloadFromDisk` refreshes are pure reads.
		// The on-disk text check keeps the write to files that genuinely still hold the
		// legacy selector, and capability-discovered project settings above are
		// read-only inputs that are never authored here.
		if (quarantineInvalid && this.#persist) {
			try {
				const projectFile = Bun.file(projectConfigPath);
				if ((await projectFile.exists()) && (await projectFile.text()).includes(LEGACY_FIREPASS_SELECTOR)) {
					await this.#writeYamlAtomically(projectConfigPath, nativeProject);
				}
			} catch (error) {
				logger.debug("Settings: could not persist project Fire Pass selector migration", { error });
			}
		}
	...
	#migrateRawSettings(raw: RawSettings, captureLegacyChangelogVersion = true): RawSettings {
		...
		// v17 tool renames (`todo_write` → `todo`, `search_tool_bm25` → `search_skills`)
		// rewritten in the settings that store tool ids. Runs before the dead-key
		// deletes below so a config carrying both eras lands fully canonical.
		migrateRenamedToolIds(raw);
		...
		// Fire Pass moved from a model-scoped Fireworks router entry to its own
		// provider, so a persisted selector naming the old entry resolves to no
		// model. Rewrite every explicit model selector a user can hold — role
		// values and retry fallback chains (keys and members) — while keeping any
		// `:thinking` suffix intact. `cycleOrder` contains role names, not models.
		const modelRoles = isRecord(raw.modelRoles) ? raw.modelRoles : undefined;
		if (modelRoles) {
			for (const role in modelRoles) {
				if (!Object.hasOwn(modelRoles, role)) continue;
				const value = modelRoles[role];
				if (typeof value === "string") modelRoles[role] = migrateFirePassSelectorValue(value);
			}
		}
		const retryObj = isRecord(raw.retry) ? raw.retry : undefined;
		const fallbackChains = isRecord(retryObj?.fallbackChains) ? retryObj.fallbackChains : undefined;
		if (retryObj && fallbackChains) {
			const migratedChains: Record<string, unknown> = {};
			for (const selector in fallbackChains) {
				if (!Object.hasOwn(fallbackChains, selector)) continue;
				const chain = fallbackChains[selector];
				migratedChains[migrateFirePassSelectorValue(selector)] = Array.isArray(chain)
					? chain.map(item => (typeof item === "string" ? migrateFirePassSelectorValue(item) : item))
					: chain;
			}
			retryObj.fallbackChains = migratedChains;
		}
