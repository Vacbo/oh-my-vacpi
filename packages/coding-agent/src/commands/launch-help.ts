...
export const launchHelp = {
	...
	flags: {
		...
		new: Flags.boolean({ description: "Start a fresh session even when auto-resume is enabled" }),
		...
		// `--goal*`: runtime parsing lives in `cli/args.ts parseArgs`, validation in
		// `modes/continuation/print-goal.ts`. Declared here so oclif's `--help` lists them and
		// so the launch parse does not reject them; the value types mirror `Args` (all strings).
		goal: Flags.string({
			description:
		...
		"goal-budget": Flags.string({ description: "Token budget for --goal (positive integer)" }),
		"goal-turns": Flags.string({ description: "Max auto-continuation turns for --goal (default 25)" }),
	},
	examples: [
		`# Interactive mode\n  ${APP_NAME}`,
