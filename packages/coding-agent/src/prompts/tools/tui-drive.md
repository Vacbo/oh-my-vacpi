...
- `start` — spawn `command` in a fresh PTY (`cwd` defaults to the agent's). Returns the session id and the screen after output settles. Sessions persist across tool calls; at most 4 concurrently.
