...
<skills-discovery>
{{discoverableSkillCount}} more skills are loaded but unlisted to keep context lean. Call `{{toolRefs.search_skills}}` describing the capability you need before assuming no skill exists; matches return `skill://<name>` URIs — read the match to apply it.
</skills-discovery>
{{/if}}
