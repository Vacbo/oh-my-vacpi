	...
	for (const entry of path) {
		if (entry.type === "thinking_level_change") {
			...
			if (entry.model) {
				...
				models[role] = normalizeFirePassSelector(entry.model);
				if (role === "default") {
			...
			//
			// The selector is normalized here, not in the message: `provider`/`model`
			// record which endpoint served that turn and stay exactly as written,
			// while the derived selector has to name a model that still resolves.
			if (!hasExplicitDefaultModel) {
				models.default = normalizeFirePassSelector(`${entry.message.provider}/${entry.message.model}`);
			}
