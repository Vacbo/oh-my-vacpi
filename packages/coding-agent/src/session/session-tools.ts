...
interface SessionToolsOptions {
	...
	/** Rebuilds the `ssh` tool after a mid-session host-config change; `null` unregisters it. */
	reloadSshTool?: () => Promise<AgentTool | null>;
	/** Caller-imposed tool allowlist; a refreshed tool outside it is registered but never activated. */
...
export class SessionTools {
	...
	constructor(host: SessionToolsHost, options: SessionToolsOptions) {
		...
		this.#requestedToolNames = options.requestedToolNames;
	 ...
	 *
	 * `/ssh add` and `/ssh remove` rewrite the host config mid-session, so the
	 * device must appear, re-describe itself, or disappear in place. Set
	 * `activateIfAvailable` when the caller just created the first host: a tool
	 * that was already registered keeps whatever presentation it had, and one
	 * excluded by {@link SessionToolsOptions.requestedToolNames} is registered
	 * but never activated.
	 */
	async refreshSshTool(options?: { activateIfAvailable?: boolean }): Promise<void> {
		resetCapabilities();
		const reloadSshTool = this.#reloadSshTool;
		...
		await this.runToolRegistryMutation(async () => {
			...
			// The outgoing tool still knows the hosts it was built from, including any the
			// caller just deleted; both sides of the edit have to leave the metadata cache.
			const candidateHostNames = new Set<string>(
				isRecord(previousSshTool) && Array.isArray(previousSshTool.hostNames)
					? previousSshTool.hostNames.filter((name: unknown): name is string => typeof name === "string")
					: [],
			);
			...
			// Descriptions render cached per-host metadata, so the cache must be dropped
			// before the reload rebuilds them - otherwise the refreshed tool advertises the
			// pre-edit address.
			...
			if (refreshedTool) {
				this.#toolRegistry.set("ssh", refreshedTool);
			} else {
				this.#toolRegistry.delete("ssh");
			}
			// A device that was already active keeps its slot, so the prompt's tool order stays
			// stable across a refresh; `activateIfAvailable` only promotes a device that did
			// not exist a moment ago.
			const activateSsh =
				refreshedTool !== null &&
				sshAllowed &&
				(wasActive || (options?.activateIfAvailable === true && !hadSshTool));
			const nextActive = previousActiveToolNames.filter(name =>
				name === "ssh" ? activateSsh : this.#toolRegistry.has(name),
			);
			if (activateSsh && !wasActive) {
				nextActive.push("ssh");
			}
			// Forced: the whole point of a refresh is that the swapped-in definition reaches
			// the model, and the registry entry can change (schema, host list) in ways the
			// applied-tool signature does not fully cover.
			await this.#applyActiveToolsByName(nextActive, true);
		});
	}
	 ...
	 * `SearchSkillsTool`, `EditTool`), and the signature reads them live on every
