...
import { generateId, migrateFirePassModelSelectors, migrateToCurrentVersion } from "./session-migrations";
import {
...
export class SessionManager {
	...
	async #setSessionFile(sessionFile: string, loadedSession?: SessionLoadResult): Promise<void> {
		...
		// Version-independent: an old Fire Pass selector can sit in a session that
		// is already at CURRENT_SESSION_VERSION, so this runs on every load and
		// persists through #rewriteRequired below.
		const firePassMigrated = migrateFirePassModelSelectors(fileEntries);
		...
		this.#fileIsCurrent = true;
		this.#rewriteRequired = migrated || firePassMigrated || loaded.malformedRecords > 0;
		this.#forceFileCreation = true;
		this.#artifactManager = null;
		this.#artifactManagerSessionFile = null;
