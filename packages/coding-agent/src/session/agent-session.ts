...
/** In-place restart accepted from the `restart` tool: `confirmation` is the
 *  message the relaunched process auto-submits as proof the new build booted. */
type RestartRequest = { confirmation: string };
...
export class AgentSession {
	...
	constructor(config: AgentSessionConfig) {
		...
		this.#tools = new SessionTools(sessionToolsHost, {
			...
			reloadSshTool: config.reloadSshTool,
			requestedToolNames: config.requestedToolNames,
	...
	/**
	 * In-place restart rail installed by {@link InteractiveMode.init} and reached
	 * by the `restart` tool through {@link ToolSession.requestRestart}. Only a
	 * top-level interactive host can re-exec the process, so print/ACP mode, an
	 * SDK embedder with no interactive host, and subagents leave it unset.
	 */
	#restartHandler: ((request: RestartRequest) => void) | undefined;

	/** Restart accepted by {@link requestRestart}, awaiting the `restart` tool
	 *  result that {@link #afterToolCall} hands to {@link #restartHandler}. */
	#pendingRestart: RestartRequest | undefined;

	/** Register or clear the host's in-place restart rail. Passing `null` clears it. */
	setRestartHandler(handler: ((request: RestartRequest) => void) | null): void {
		this.#restartHandler = handler ?? undefined;
		if (!handler) this.#pendingRestart = undefined;
	}

	/**
	 * The `restart` tool's entry point, undefined until a host registers a rail.
	 * Its absence is what makes the tool report itself unavailable before it
	 * boot-probes the relaunch artifact.
	 *
	 * Accepting only records the request. {@link #afterToolCall} hands it to the
	 * host after the tool result is finalized, so teardown cannot preempt the
	 * result the confirmation refers to.
	 */
	get requestRestart(): ((request: RestartRequest) => boolean) | undefined {
		return this.#restartHandler ? this.#acceptRestart : undefined;
	}

	readonly #acceptRestart = (request: RestartRequest): boolean => {
		if (!this.#restartHandler) return false;
		...
		if (event.type === "message_end") {
			...
			if (event.message.role === "toolResult") {
				...
				if (toolName === "todo" && isError) {
					...
					const reminderText = [
						"<system-reminder>",
						"The todo tool failed, so todo progress is not visible to the user.",
						errorText ? `Failure: ${errorText}` : "Failure: the todo tool returned an error.",
	...
	#afterToolCall(ctx: AfterToolCallContext): AfterToolCallResult | undefined {
		...
		if (ctx.toolCall.name === "restart" && this.#pendingRestart) {
			const request = this.#pendingRestart;
			this.#pendingRestart = undefined;
			// Nothing may run in a process image about to be replaced, so the turn
			// ends here like a terminal tool result. An errored result drops the
			// request and leaves the session on the current build.
			if (!ctx.isError) {
				const handler = this.#restartHandler;
				this.agent.abort(TERMINAL_TOOL_RESULT_ABORT_REASON);
				// Detached from this hook's frame so host teardown neither runs inside
				// nor throws through agent-core finalization.
				queueMicrotask(() => {
					try {
						handler?.(request);
					} catch (err) {
						logger.error("Restart handler failed", { err });
					}
				});
			}
		}
	...
	/** Rebuilds the ssh device from disk-backed capability discovery. */
