 ...
 *  turn-1 switches in practice. `todo_write` is deliberately NOT a trigger:
 *  firing at the todo init handed the fast model 100% of the implementation
 *  with zero started work and measurably regressed pass rates. */
...
export interface AgentSessionConfig {
	...
	/** Non-MCP discovered tool names (BM25-activated built-ins) restored from the session branch. */
...
export class AgentSession {
	...
	/**
	 * Whether the `todo_write` tool can actually be used this run: active in the
	 * request, or hidden-but-discoverable via `search_tool_bm25`. Registry
	 * membership alone is insufficient — a default-inactive registry entry the
	 * model can neither call nor discover produces no todo list, so the prewalk
	 * todo gate must not wait on it. A disabled `todo_write` is never callable
	 * regardless of how the active set was populated. Mirrors the availability
	 * predicate in {@link #buildGoalTodoContext}.
	 */
	#isTodoToolCallable(): boolean {
		if ((this.settings.get("tools.disabledTools") ?? []).includes("todo_write")) return false;
		if (this.getActiveToolNames().includes("todo_write")) return true;
		return this.getDiscoverableTools({ source: "builtin" }).some(tool => tool.name === "todo_write");
	}
	...
	async #advancePrewalk(liveMessages: AgentMessage[], context: AgentTurnEndContext | undefined): Promise<void> {
		...
		// the fast model the whole implementation cold. Registry membership is
		// not enough — the gate only applies when `todo_write` is actually
		// callable this run (active, or discoverable via BM25). When it is
		// disabled or otherwise neither active nor discoverable, there is no
		// todo list to wait for, so the gate opens immediately.
		if (context.toolResults.some(result => result.toolName === "todo_write")) {
			this.#prewalkTodoSeen = true;
		}
		const todoGateOpen = this.#prewalkTodoSeen || !this.#isTodoToolCallable();
	...
	constructor(config: AgentSessionConfig) {
		...
		this.#selectedDiscoveredToolNames = new Set(config.initialSelectedDiscoveredToolNames ?? []);
		...
		const persistedSelectedToolNames = this.buildDisplaySessionContext().selectedMCPToolNames;
		const currentSelectedToolNames = this.getSelectedDiscoveredToolNames();
		...
		if (
			...
			!this.#selectedMCPToolNamesMatch(persistedSelectedToolNames, currentSelectedToolNames)
		) {
			this.sessionManager.appendMCPToolSelection(currentSelectedToolNames);
		}
	...
	#processAgentEvent = async (event: AgentEvent): Promise<void> => {
		...
		// freshest counter — otherwise a turn that just invoked `todo_write` could
	...
	#persistSelectedToolNamesIfChanged(previousSelectedToolNames: string[]): void {
		// Persist when either discovery surface (MCP hiding or discovery "all" built-in
		// hiding) is active; the entry stores the union of both selection kinds.
		if (!this.#mcpDiscoveryEnabled && this.#resolveEffectiveDiscoveryMode() !== "all") return;
		const nextSelectedToolNames = this.getSelectedDiscoveredToolNames();
		if (this.#selectedMCPToolNamesMatch(previousSelectedToolNames, nextSelectedToolNames)) {
			return;
		}
		this.sessionManager.appendMCPToolSelection(nextSelectedToolNames);
	}
	...
	async activateVibeTools(baseToolNames: string[]): Promise<void> {
		...
		await this.#applyActiveToolsByName([...new Set([...baseToolNames, ...vibeToolNames])], { temporarySlate: true });
	}
	...
	async deactivateVibeTools(nextToolNames: string[]): Promise<void> {
		...
		await this.#applyActiveToolsByName(nextToolNames, { temporarySlate: true });
	}
	...
	/**
	 * Apply an active tool set by name, re-syncing discovery selection state and
	 * persisting it.
	 *
	 * `temporarySlate` marks a transient tool-mode slate (e.g. `/vibe`'s
	 * read-only + vibe-tool set) that must swap the live tools WITHOUT touching
	 * the durable discovery selections: it skips the `#selectedMCPToolNames`
	 * rewrite, the `#selectedDiscoveredToolNames` prune, and persistence. This
	 * keeps BM25-activated builtins and MCP selections intact across the
	 * enter/exit round-trip so a later resume restores exactly what the user had
	 * before the slate. Normal calls keep the mutate-and-persist behavior.
	 */
	async #applyActiveToolsByName(
		toolNames: string[],
		options?: { persistMCPSelection?: boolean; previousSelectedToolNames?: string[]; temporarySlate?: boolean },
	): Promise<void> {
		toolNames = normalizeToolNames(toolNames);
		const previousSelectedToolNames = options?.previousSelectedToolNames ?? this.getSelectedDiscoveredToolNames();
		...
		if (this.#mcpDiscoveryEnabled && !options?.temporarySlate) {
		...
		if (options?.persistMCPSelection !== false && !options?.temporarySlate) {
			this.#persistSelectedToolNamesIfChanged(previousSelectedToolNames);
		}
	}
	...
	async #restoreSelectionsForSessionContext(
		sessionContext: SessionContext,
		options?: { fallbackSelectedMCPToolNames?: Iterable<string> },
	): Promise<void> {
		const allMode = this.#resolveEffectiveDiscoveryMode() === "all";
		if (!this.#mcpDiscoveryEnabled && !allMode) return;
		const nextActiveNonMCPToolNames = this.#getActiveNonMCPToolNames();
		// Built-in (non-MCP) activations persisted in the selection entry. Only
		// meaningful in "all" mode; registry-gated so stale entries cannot
		// activate ghost tools.
		const restoredDiscoveredToolNames = allMode
			? sessionContext.selectedMCPToolNames.filter(name => !isMCPToolName(name) && this.#toolRegistry.has(name))
			: [];
		if (allMode) {
			this.#selectedDiscoveredToolNames = new Set(restoredDiscoveredToolNames);
		}
		if (!this.#mcpDiscoveryEnabled) {
			// MCP discovery off: MCP tools are plain active tools; leave them untouched.
			const activeMCPToolNames = this.getActiveToolNames().filter(name => isMCPToolName(name));
			await this.#applyActiveToolsByName(
				[...nextActiveNonMCPToolNames, ...restoredDiscoveredToolNames, ...activeMCPToolNames],
		...
		await this.#applyActiveToolsByName(
			[...nextActiveNonMCPToolNames, ...restoredDiscoveredToolNames, ...restoredMCPToolNames],
			{ persistMCPSelection: false },
		);
	}
	...
	async refreshMCPTools(mcpTools: CustomTool[], options?: { activateAll?: boolean }): Promise<void> {
		const previousSelectedToolNames = this.getSelectedDiscoveredToolNames();
		...
		if (options?.activateAll) {
			...
			await this.#applyActiveToolsByName(nextActive, { previousSelectedToolNames });
			return;
		}
	...
	#buildGoalTodoContext(): string | undefined {
		...
		const activeToolNames = this.getActiveToolNames();
		const canCallTodoTool = activeToolNames.includes("todo_write");
		const canDiscoverTodoTool =
			!canCallTodoTool && this.getDiscoverableTools({ source: "builtin" }).some(tool => tool.name === "todo_write");
	...
	async newSession(options?: NewSessionOptions): Promise<boolean> {
		...
		if (nextDiscoverySessionToolNames) {
			...
			const selectedToolNames = this.getSelectedDiscoveredToolNames();
			if (selectedToolNames.length > 0) {
				this.sessionManager.appendMCPToolSelection(selectedToolNames);
			}
		}
		this.#rememberSessionDefaultSelectedMCPToolNames(
			this.sessionFile,
	...
	async #applyRewind(report: string, activeMessages?: AgentMessage[]): Promise<void> {
		...
		await this.#restoreSelectionsForSessionContext(sessionContext);
	...
	#buildEagerPreludeContext(): { toolRefs: Record<string, string>; taskBatch: boolean } {
		...
		return {
			toolRefs: { task: wireName("task"), todo: wireName("todo_write") },
	...
	#createEagerTodoPrelude(
		...
		// (tools.discoveryMode === "all") can register `todo_write` while hiding it from
		// the exposed tools. Forcing a named tool_choice for an inactive tool makes
		// the provider reject the request (HTTP 400).
		if (!this.getActiveToolNames().includes("todo_write")) {
			logger.warn("Eager todo enforcement skipped because todo_write is not active", {
		...
		// `todo_write` tool on the first turn — the previous boolean-on behavior. Post-compaction
	...
	#takeMidRunTodoNudge(): AgentMessage | null {
		...
		if (this.#planModeState?.enabled) return null;
		// Tool discovery / explicit active-tool lists can hide `todo_write` from this
		...
		if (!this.getActiveToolNames().includes("todo_write")) return null;
	...
	async switchSession(sessionPath: string): Promise<boolean> {
		...
		try {
			...
			await this.#restoreSelectionsForSessionContext(sessionContext, { fallbackSelectedMCPToolNames });
			this.#rehydrateCheckpointRewindState();
			...
			try {
				...
				// state so selection restoration still has its inputs.
				const selectionRestoreContext = previousSessionContext ?? this.buildDisplaySessionContext();
				await this.#restoreSelectionsForSessionContext(selectionRestoreContext, {
				...
				this.#selectedDiscoveredToolNames = new Set(previousSelectedDiscoveredToolNames);
	...
	async navigateTree(
		...
		await this.#restoreSelectionsForSessionContext(displayContext);
