 ...
 * v7 unifies the fork's v6 (slash_command_usage) with upstream's v6 (model_perf):
 * both parents shipped v6 independently, so the v6 -> v7 step reconciles either
 * lineage's v6 database (see #migrateSchemaV6ToV7).
 */
...
`);
	...
	/**
	 * v6 → v7: reconcile the two independently-shipped v6 schemas. The fork's v6
	 * added slash_command_usage; upstream's v6 added model_perf/meta. A v6 db from
	 * either lineage is therefore missing the other side's table. Only
	 * slash_command_usage needs an explicit create here — model_perf and meta are
	 * (re)created by #initializeSchema's CREATE TABLE IF NOT EXISTS. Critically, we
	 * do NOT purge model_perf on this step: an upstream v6 db already holds valid
	 * total-duration TPS aggregates, and wiping them would force a needless
	 * re-backfill and drop live-only samples. Idempotent for fork v6 dbs where the
	 * table already exists.
	 */
	#migrateSchemaV6ToV7(): void {
		this.#db.run(`
CREATE TABLE IF NOT EXISTS slash_command_usage (
	command_name TEXT PRIMARY KEY,
	last_used_at INTEGER NOT NULL DEFAULT (${SQLITE_NOW_EPOCH})
