...
export interface AgentSessionConfig {
	...
	/**
	 * Rebuilds the `ssh` tool from disk-backed capability discovery, or resolves
	 * `null` once no host remains and the tool should be unregistered.
	 *
	 * SSH hosts are edited while the session is live (`/ssh add`, `/ssh remove`),
	 * so the device has to appear, change, or disappear in place — restarting omp
	 * to pick up a new host is not an acceptable workflow. Drives
	 * {@link AgentSession.refreshSshTool}.
	 */
	reloadSshTool?: () => Promise<AgentTool | null>;
	/**
	 * Explicit tool allowlist the session was opened with; `undefined` means the
	 * caller imposed none.
	 *
	 * A mid-session ssh refresh still registers the rebuilt device so `/ssh`
	...
	/**
	 * Live-run registration this session reports into (`omp sessions`,
	 * `tui_observe`/`tui_drive` discovery). Owned by the SDK factory: the session
	 * refreshes its metadata on session-id changes and marks it stopped on dispose.
	 */
