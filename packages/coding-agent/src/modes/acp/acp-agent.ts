...
export class AcpAgent implements Agent {
	...
	#replayToolResult(
		...
		// Sessions recorded before the todo rename carry `todo_write`. Replay feeds
		// synthesized events through the same mapper as live turns, and that mapper
		// recognizes only the canonical `todo`, so normalize the historical spelling
		// here — otherwise an old transcript replays without its plan update.
		const toolName = isHistoricalTodoToolName(message.toolName) ? "todo" : message.toolName;
