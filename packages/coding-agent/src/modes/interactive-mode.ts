...
import { installRestartRail } from "./restart-rail";
...
/**
 * Mirror every write of `inner` to `onWrite` while the rest of the `Terminal`
 * surface keeps reaching the real terminal untouched.
 *
 * The wrapper is installed over a terminal the composer may already have
 * started, so it must forward members a hand-written forwarder would predate —
 * `enableInput()` (the deferred-input replay of keystrokes buffered during
 * startup), `pendingOutputBytes`, the keyboard-enhancement sequences, and the
 * appearance/private-mode probes. Reflective delegation absorbs that drift;
 * methods and getters are invoked on the real terminal because its private
 * fields are unreachable through a proxy receiver.
 */
function tapTerminalWrites(inner: Terminal, onWrite: (data: string) => void): Terminal {
	...
	return new Proxy(inner, {
		get(target, property) {
			...
			const bound = boundMembers.get(property);
			if (bound !== undefined) return bound;
			const value = Reflect.get(target, property, target);
			if (typeof value !== "function") return value;
			const forward = (value as (...args: unknown[]) => unknown).bind(target);
			boundMembers.set(property, forward);
			return forward;
		},
...
function shouldEnterPlanModeOnStartup(
...
export class InteractiveMode implements InteractiveModeContext {
	...
	/** Follow-up the relaunched process auto-submits after resuming, set when the
	 *  `restart` tool (not `/restart`) asked for the re-exec. */
	#restartConfirmation: string | undefined;
	/** Unregisters the `restart` tool's rail so a retained session cannot reach a
	 *  torn-down mode. Set by {@link init}, consumed by {@link #teardown}. */
	#restartRailDispose: (() => void) | undefined;
	...
	constructor(
		...
		// buffered during startup remains in the same editor instance, and tap
		// its writes in place rather than building a second ProcessTerminal.
		// Optional call: upstream-derived test harnesses construct partial session
		// doubles without the fork's live-session registry surface.
		const liveSession = session.getLiveSession?.();
		if (liveSession) {
			const baseTerminal = this.ui.terminal;
			...
			this.ui.terminal = tapTerminalWrites(baseTerminal, data => {
	...
	async init(options: InteractiveModeInitOptions = {}): Promise<void> {
		...
		// Wire the issue-report consent gate to the Yes/No dialog popup. The
		// handler is process-global — the `xd://report_issue` device (including
		// subagent invocations, which can't reach `showHookSelector` on their own)
		// resolves through this exact closure. `Settings.instance` is the
		// disk-backed singleton; passing it explicitly guarantees the decision
		// persists even when the prompt is triggered from a subagent whose own
		// `Settings` is an in-memory snapshot.
		...
		// The confirmation only reaches the relaunched process as the positional
		// `restart()` appends to the rewritten launch argv, so stash it before
		// teardown starts.
		this.#restartRailDispose = installRestartRail(this.session, async confirmation => {
			this.#restartConfirmation = confirmation;
			await this.restart();
		});
	...
	#hidePlanReview(): void {
		const wasOpen = this.#planReviewOverlay !== undefined;
		...
		// `planReviewActive` suppresses idle compaction while the overlay owns the
		// context decision (compact / keep / fresh session). Re-arm the timer here —
		// the single place every close path funnels through — or the suppression
		// outlives the overlay for the rest of the session.
		if (wasOpen) this.#eventController.rescheduleIdleCompaction();
	}
	...
	/**
	 * Show the issue-report consent popup and return the user's decision.
	 * Invoked by the process-global consent handler the `xd://report_issue`
	 * device dispatches to; subagent invocations bubble up here through the
	 * shared module state.
	 */
	 ...
	 * the recorder first so a late tapped write cannot re-arm its scheduled
	...
	async shutdown(): Promise<void> {
		...
		// #resumableSession).
		const resumableSession = this.#resumableSession();
		if (resumableSession) {
			process.stderr.write(`\n${chalk.dim(`Resume this session with ${resumeCommand(resumableSession.id)}`)}\n`);
		}
	...
	async restart(): Promise<void> {
		...
		// Resolve the resumable session only now: the teardown's flush above is
		// what materializes a fresh session's file on disk, so reading it any
		// earlier relaunches into a new session instead of resuming.
		const resumeTarget = this.#resumableSession()?.file;
		const cmd = [...resolveCliEntryCmd(), ...restartArgv(process.argv.slice(2), resumeTarget)];
		// A restart asked for by the `restart` tool (never `/restart`) carries a
		// confirmation the resumed session auto-submits as the boot proof; it rides
		// along as the sole positional, since `restartArgv` strips the original
		// ones. With no session to resume there is nothing to prove into, so the
		// confirmation is dropped instead of becoming a stray initial prompt.
		if (resumeTarget !== undefined && this.#restartConfirmation !== undefined) {
			cmd.push(this.#restartConfirmation);
		}
	...
	/**
	 * Durable session identity and exact file path, or `undefined` when lazy
	 * persistence never materialized the allocated path. Restart uses the path
	 * so sessions opened outside configured/default roots remain resumable;
	 * shutdown uses the compact id in its human-facing command hint.
	 */
	#resumableSession(): { id: string; file: string } | undefined {
		const id = this.sessionManager.getSessionId();
		const file = this.sessionManager.getSessionFile();
		return id && file && this.sessionManager.isSessionOnDisk() ? { id, file } : undefined;
	}

	/** Shared `shutdown()`/`restart()` teardown: dispose the session and hand the terminal back. */
	async #teardown(): Promise<void> {
		// Before any teardown: a session retained past this mode must not hold a
		// restart closure that can no longer relaunch anything.
		this.#restartRailDispose?.();
		this.#restartRailDispose = undefined;
	...
	showUpstreamMergeAvailableNotification(upstreamVersion: string): void {
		this.#uiHelpers.showUpstreamMergeAvailableNotification(upstreamVersion);
	}
	...
	/**
	 * `/tools` has two faces, split on the turn lifecycle rather than duplicated:
	 * the interactive Tool Control Center toggles `tools.disabledTools` on the
	 * live session, so it is only offered while no turn owns the tool set. During
	 * a streamed turn the command degrades to the read-only inventory panel,
	 * which `presentCommandOutput` defers and mounts exactly once when the turn
	 * settles (issue #4806), instead of popping a mutating overlay over the
	 * active frame.
	 */
