...
export class EventController {
	...
	async handleEvent(event: AgentSessionEvent): Promise<void> {
		...
		// (idle Working loader, Markdown fences, wrapping, tool previews). The TUI
		// can use its foreground live-region path instead of idle deferral, which
		// keeps history cleaner and avoids leaving the loader/status frame frozen
		// until the next input, but may yank readers who used native mouse
		// scrollback on terminals that cannot report viewport position. Keep that
		// behavior opt-in through `tui.rebuildScrollbackDuringStreaming`.
