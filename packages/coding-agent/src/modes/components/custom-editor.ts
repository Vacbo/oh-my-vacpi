 ...
 */
function pickEditorTheme(args: readonly unknown[]): EditorTheme | undefined {
...
export class CustomEditor extends Editor {
	 ...
	 * keep working.
	 *
	 * A missing or placeholder theme (plugin host, pre-init render, a fresh
	 * process before `initTheme`) falls back to {@link getEditorTheme}, which
	 * yields the active theme once initialized and a self-contained plain-text
	 * default before that. That fallback needs no initialized global settings, so
	 * `new CustomEditor()` is always usable.
	 */
	constructor(...args: readonly unknown[]) {
		super(args[0], pickEditorTheme(args) ?? getEditorTheme(), args[2]);
