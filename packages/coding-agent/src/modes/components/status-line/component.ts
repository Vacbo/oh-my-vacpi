...
const DISPLAY_REPO_OPTIONS = { preferJj: true } as const;
...
export class StatusLineComponent implements Component {
	...
	#setupGitWatcher(): void {
		...
		const repository = vcs.repo(effectiveGitCwd, DISPLAY_REPO_OPTIONS);
		if (!repository) {
	...
	#getBranchLabel(effectiveGitCwd?: string): string | null {
		...
		const repository = vcs.repo(gitCwd, DISPLAY_REPO_OPTIONS);
	...
	#lookupPr(effectiveGitCwd?: string): { number: number; url: string } | null {
		...
		const gitCwd = effectiveGitCwd ?? this.#resolveActiveRepoCache().effectiveGitCwd;
		if (vcs.repo(gitCwd, DISPLAY_REPO_OPTIONS)?.kind() !== "git") return null;
		const branch = this.#getBranchLabel(gitCwd);
		const currentContext = branch ? createPrCacheContext(branch, this.#cachedBranchRepoId ?? null) : null;

		const stalePr = this.#cachedPr;

		// A lookup is already in flight; keep the last value visible to avoid flicker.
		if (this.#prLookupInFlight) {
			return stalePr ?? null;
		}

		// Terminal cases: no PR can apply here — no branch, detached HEAD, or the
		// default branch (a jj working copy already returned above: it exposes no
		// git branch for `gh pr view` to resolve). Clear any cached PR so one from
		// a previous branch can't linger. This runs before the cache-reuse check
		// below: the default-branch status resolves async, so a PR cached during
		// that window would otherwise be reused here and never cleared
		// (invalidateGitCaches drops only the PR context, not the PR).
		if (!branch || branch === "detached" || this.#isDefaultBranch(branch, gitCwd)) {
			this.#cachedPr = null;
			this.#cachedPrContext = undefined;
			return null;
		}
