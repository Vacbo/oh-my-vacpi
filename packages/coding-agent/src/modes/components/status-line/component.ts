...
export class StatusLineComponent implements Component {
	...
	#getCurrentBranch(effectiveGitCwd?: string): string | null {
		...
		const jjLabel = jj.headLabel.getSync(gitCwd);
	...
	#lookupPr(effectiveGitCwd?: string): { number: number; url: string } | null {
		...
		// Terminal cases: no PR can apply here — no branch, detached HEAD, a jj working
		// copy (which exposes no git branch for `gh pr view` to resolve), or the default
		// branch. Clear any cached PR so one from a previous branch can't linger. This
		// runs before the cache-reuse check below: the default-branch status resolves
		// async, so a PR cached during that window would otherwise be reused here and
		// never cleared (#invalidateGitCaches drops only the PR context, not the PR).
		if (
			!branch ||
			branch === "detached" ||
			jj.repo.rootSync(gitCwd) !== null ||
			this.#isDefaultBranch(branch, gitCwd)
		) {
			this.#cachedPr = null;
			this.#cachedPrContext = undefined;
			return null;
