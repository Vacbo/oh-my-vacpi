...
import { type InspectorMeta, InspectorPanel, type ToolRuntimeSource } from "./inspector-panel";
...
import { type DashboardState, type Extension, isShadowedExtension, type ProviderTab } from "./types";
...
function extFooter(): string {
	return ` ↑/↓: navigate · Space: toggle · ^p: pin skill · ←/→: provider · PgUp/PgDn: inspector · ${expandKeyHint()}: expand · Esc: close`;
}
