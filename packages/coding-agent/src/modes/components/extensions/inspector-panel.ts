 ...
 * identity → runtime/enablement → pin state → description → origin →
...
export class InspectorPanel implements Component {
	...
	/** Caller-supplied note on session-level availability (Tool Control Center). */
	#pushStatusNote(lines: string[]): void {
		if (this.#meta.statusNote) lines.push(`  ${theme.fg("dim", this.#meta.statusNote)}`);
	}

	/**
	 * Pin state, rendered next to enablement because it is the same class of
	 * fact. Callers opt in by setting {@link InspectorMeta.pinned}; kinds that
	 * cannot pin leave it undefined and the section disappears.
	 */
	#pushPinState(lines: string[], ext: Extension): void {
		if (this.#meta.pinned === undefined) return;
