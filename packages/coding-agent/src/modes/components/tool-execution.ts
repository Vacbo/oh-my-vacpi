...
function displaceableToolName(
	...
	if (toolName === "todo_write" && !isPartial && isTodoToolDetails(result.details)) return "todo";
	return undefined;
}
...
export class ToolExecutionComponent extends Container implements NativeScrollbackLiveRegion {
	...
	updateResult(
		...
		// Detached task progress snapshots and other partial updates must not
		// repaint once their block has left the transcript live region. The task
		// path freezes its static progress chrome first; the generic guard then
		// drops any partial update already eligible for native scrollback.
		if (isPartial && this.#toolName === "task" && this.#maybeFreezeBackgroundTask()) return;
