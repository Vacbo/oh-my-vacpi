...
import { isHistoricalTodoToolName, TODO_STRIKE_TOTAL_FRAMES, type TodoToolDetails } from "../../tools/todo";
...
function displaceableToolName(
	...
	if (isHistoricalTodoToolName(toolName) && !isPartial && isTodoToolDetails(result.details)) return "todo";
	return undefined;
}
...
export class ToolExecutionComponent extends Container implements NativeScrollbackLiveRegion {
	...
	updateResult(
		...
		// The generic guard then drops any other partial update whose block has
		// already left the transcript live region (e.g. a background bash
		// progress snapshot) — it is eligible for native scrollback and must
		// not repaint.
