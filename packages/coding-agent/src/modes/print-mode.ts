...
export async function runPrintMode(session: AgentSession, options: PrintModeOptions): Promise<number> {
	const { mode, messages = [], initialMessage, initialImages, goal, printThoughts, planYolo = false } = options;

	if (goal && !session.settings.get("goal.enabled")) {
		process.stderr.write("--goal requires the goal.enabled setting\n");
		// Rejecting the run still owns a live session: tear it down on the same
		// budgeted path as a normal exit so telemetry flushes and the browser
		// reaper runs instead of leaking an OMP-owned Chromium.
	...
	// primary turn whose response print mode would never emit. Goal continuation
	// above owns the last primary turn, so this lands after the pump.
