...
describe("Settings", () => {
	...
	describe("migrations", () => {
		...
		it("rewrites legacy Fire Pass selectors in persisted model settings", async () => {
			await writeSettings({
				modelRoles: {
					default: `${LEGACY_FIREPASS_SELECTOR}:high`,
					smol: "anthropic/claude-haiku-4-5",
				},
				cycleOrder: [LEGACY_FIREPASS_SELECTOR, "default"],
				retry: {
					fallbackChains: {
						[`${LEGACY_FIREPASS_SELECTOR}:max`]: [LEGACY_FIREPASS_SELECTOR, "openai/gpt-5-mini"],
					},
				},
			});
			...
			expect(settings.get("modelRoles")).toMatchObject({
				default: `${FIREPASS_SELECTOR}:high`,
				smol: "anthropic/claude-haiku-4-5",
			});
			expect(settings.get("cycleOrder")).toEqual([LEGACY_FIREPASS_SELECTOR, "default"]);
			expect(settings.get("retry.fallbackChains")).toEqual({
				[`${FIREPASS_SELECTOR}:max`]: [FIREPASS_SELECTOR, "openai/gpt-5-mini"],
			});
			...
			const onDisk = await readSettings();
			expect(JSON.stringify((onDisk.modelRoles as Record<string, unknown>) ?? {})).not.toContain(
				LEGACY_FIREPASS_SELECTOR,
			);
			expect(JSON.stringify((onDisk.retry as Record<string, unknown>) ?? {})).not.toContain(
				LEGACY_FIREPASS_SELECTOR,
			);
			expect(JSON.stringify(onDisk)).toContain(FIREPASS_SELECTOR);

			const reloaded = await Settings.loadIsolated({ cwd: projectDir, agentDir });
			expect(reloaded.get("modelRoles").default).toBe(`${FIREPASS_SELECTOR}:high`);
			expect(reloaded.get("cycleOrder")[0]).toBe(LEGACY_FIREPASS_SELECTOR);
			expect(reloaded.get("retry.fallbackChains")[`${FIREPASS_SELECTOR}:max`]).toEqual([
				FIREPASS_SELECTOR,
				"openai/gpt-5-mini",
			]);
		});

		it("rewrites renamed tool ids in tool-id arrays and persists the canonical list", async () => {
			await writeSettings({
				tools: {
					// Both eras in one list: the rename must not leave a duplicate behind.
					disabledTools: ["todo_write", "browser", "todo", "search_tool_bm25"],
					xdevInlineDevices: ["todo_write", "mcp__context_mode_*"],
				},
			});

			const settings = await Settings.init({ cwd: projectDir, agentDir });
			expect(settings.get("tools.disabledTools")).toEqual(["todo", "browser", "search_skills"]);
			expect(settings.get("tools.xdevInlineDevices")).toEqual(["todo", "mcp__context_mode_*"]);
			...
			const onDisk = await readSettings();
			const tools = onDisk.tools as Record<string, unknown>;
			expect(tools.disabledTools).toEqual(["todo", "browser", "search_skills"]);
			expect(tools.xdevInlineDevices).toEqual(["todo", "mcp__context_mode_*"]);
			expect(JSON.stringify(onDisk)).not.toContain("todo_write");
			expect(JSON.stringify(onDisk)).not.toContain("search_tool_bm25");

			const reloaded = await Settings.loadIsolated({ cwd: projectDir, agentDir });
			expect(reloaded.get("tools.disabledTools")).toEqual(["todo", "browser", "search_skills"]);
		});

		it("rewrites renamed tool ids stored under quoted-dotted keys", async () => {
			await writeSettings({ "tools.disabledTools": ["search_tool_bm25", "todo_write"] });

			const settings = await Settings.init({ cwd: projectDir, agentDir });
			expect(settings.get("tools.disabledTools")).toEqual(["search_skills", "todo"]);
		});

		it("leaves an already-canonical tool-id array untouched", async () => {
			const settings = Settings.isolated({ "tools.disabledTools": ["browser", "todo"] });
			expect(settings.get("tools.disabledTools")).toEqual(["browser", "todo"]);
		});
