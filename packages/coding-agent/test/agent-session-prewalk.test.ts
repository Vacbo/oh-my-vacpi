...
describe("AgentSession prewalk", () => {
	...
	it("does not wait on a todo_write that is registered but neither active nor discoverable", async () => {
		// Regression: the todo gate keyed off registry membership
		// (`toolRegistry.has("todo_write")`). A `todo_write` present in the
		// registry but inactive and not discoverable (loadMode !==
		// "discoverable") can neither be called nor found by the model, so it
		// produces no todo list to wait on. Membership must NOT hold the gate
		// closed — the first edit/write has to switch. The tool is left in the
		// shared registry (so `has("todo_write")` is true) but excluded from the
		// agent's active tools, isolating callability from mere registration.
		const primary = modelOrThrow("claude-sonnet-4-5");
		const target = modelOrThrow("claude-sonnet-4-6");
		const modelRegistry = new ModelRegistry(authStorage, path.join(tempDir.path(), "models.yml"));

		// Turn 1: read-only (nudge after). Turn 2: write — gate is already open
		// because todo_write is uncallable, so this first write switches.
		const mock = createMockModel({
		...
		const agent = new Agent({
			...
			initialState: {
				model: primary,
				systemPrompt: ["Test"],
				// No todoTool here: `todo_write` is registered but inactive.
	...
	it("does not wait on a todo_write that is active but disabled via tools.disabledTools", async () => {
		// Regression: a `todo_write` the model can nominally see but that is
		// disabled produces no usable todo list, so the gate must open. The
		// disabled guard has to win even when the tool is otherwise active —
		// the first edit/write switches without waiting for a todo call.
		const primary = modelOrThrow("claude-sonnet-4-5");
		const target = modelOrThrow("claude-sonnet-4-6");
		const modelRegistry = new ModelRegistry(authStorage, path.join(tempDir.path(), "models.yml"));
		...
		const agent = new Agent({
			...
			initialState: {
				model: primary,
				systemPrompt: ["Test"],
				// todo_write is active here, but disabled below — the guard must
				// still treat it as uncallable.
				tools: [recordTool as AgentTool, writeTool as AgentTool, todoTool as AgentTool],
				messages: [],
				thinkingLevel: Effort.Medium,
			},
		...
		session = new AgentSession({
			agent,
			sessionManager: SessionManager.inMemory(),
			settings: Settings.isolated({ "compaction.enabled": false, "tools.disabledTools": ["todo_write"] }),
			modelRegistry,
			toolRegistry,
			prewalk: { target },
		});
