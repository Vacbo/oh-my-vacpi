...
describe("StatusLineComponent PR lookup timeout guard", () => {
	...
	it("suppresses PR lookup when a colocated workspace prefers Jujutsu display state", () => {
		const jjRepository = {
			kind: () => "jj",
			asGit: () => null,
			asJj: () => null,
			root: () => fakeRepoInfo.repoRoot,
			watchTarget: () => "/fake/.jj/repo/op_heads/heads",
			label: async () => "feature",
		} as unknown as VcsRepo;
		const repoSpy = vi
			.spyOn(vcs, "repo")
			.mockImplementation((_cwd, options) => (options?.preferJj ? jjRepository : null));
		const githubSpy = vi.spyOn(github, "run").mockResolvedValue({ exitCode: 1, stdout: "", stderr: "" });

		const component = new StatusLineComponent(makeSession());
		component.updateSettings(gitSegmentSettings);
		try {
			component.getTopBorder(80);
			expect(repoSpy).toHaveBeenCalledWith(expect.any(String), { preferJj: true });
