...
describe("ToolExecutionComponent live preview spinners", () => {
	...
	it("does not tick partial todo snapshots and routes them through the dedicated renderer", () => {
		vi.useFakeTimers();
		const requestRender = vi.fn();
		const requestComponentRender = vi.fn();
		// The todo tool is registered as `todo`; the dedicated renderer +
		// static-spinner exception both key off that wire name. A live todo
		// call must render its static "Todo" status line (not the generic
		// tool-name fallback) and start no spinner interval.
		const component = new ToolExecutionComponent(
			"todo",
