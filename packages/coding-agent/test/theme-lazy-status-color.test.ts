...
describe("lazy status color re-resolves on theme switch", () => {
	...
	it("recolors the presented upstream-merge notification when auto-theme resolves light", async () => {
		themeModule.onTerminalAppearanceChange("dark");
		await themeModule.initTheme(false, undefined, undefined, "dark-catppuccin", "light-catppuccin");
		...
		const context: Pick<InteractiveModeContext, "present"> = {
			present(component) {
				if (!isSingleComponent(component)) throw new Error("Expected one upstream notification block");
				presented = component;
			},
		};
		new UiHelpers(context as InteractiveModeContext).showUpstreamMergeAvailableNotification("1.2.3");
		const notification = presented;
		if (!notification) throw new Error("Upstream notification was not presented");
		...
		const semanticLines = Bun.stripANSI(lightOutput)
			.split("\n")
			.map(line => line.trim())
			.filter(line => line === "Upstream Release Available" || line.startsWith("Upstream 1.2.3"));
		expect(semanticLines).toEqual([
			"Upstream Release Available",
			"Upstream 1.2.3 is available to merge into this fork.",
		]);
	});
});
