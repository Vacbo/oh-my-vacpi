...
describe("SessionManager JSONL software-crash durability", () => {
	...
	it("rewrites current-version Fire Pass selectors when reopening a session", async () => {
		const cwd = makeTempDir("@pi-firepass-session-cwd-");
		const manager = SessionManager.create(cwd, path.join(cwd, "sessions"));
		manager.appendModelChange(LEGACY_FIREPASS_SELECTOR, "default");
		...
		const resumed = await SessionManager.open(sessionFile);
		expect(resumed.getEntries().find(entry => entry.type === "model_change")).toMatchObject({
			model: FIREPASS_SELECTOR,
		});

		resumed.appendMessage({ role: "user", content: "after migration", timestamp: Date.now() });
		expect(readJsonl(sessionFile).find(entry => entry.type === "model_change")).toMatchObject({
			model: FIREPASS_SELECTOR,
		});
		await resumed.close();
	});
