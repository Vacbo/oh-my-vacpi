/**
 * Consent gate around the `xd://report_issue` device. Asserts:
 *
 * 1. With no handler registered, consent defaults to `false`, so a device
 *    write is dropped without touching the DB.
