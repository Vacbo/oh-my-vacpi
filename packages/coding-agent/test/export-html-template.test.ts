...
const expectedSourceTemplate = composeExpectedTemplate();
const expectedTemplate: TemplateProbeResult = {
	chars: expectedSourceTemplate.length,
	bytes: new TextEncoder().encode(expectedSourceTemplate).byteLength,
	sha256: new Bun.CryptoHasher("sha256").update(expectedSourceTemplate).digest("hex"),
	stableCache: true,
	assetsRemoved: 0,
};
