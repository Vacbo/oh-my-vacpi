...
describe("UiHelpers.renderSessionContext collapses repeated todo snapshots", () => {
	...
	it("removes the earlier todo snapshot when an assistant message replays two todo calls", () => {
		...
		const assistant = {
			role: "assistant",
			content: [
				{ type: "toolCall", id: "todo-1", name: "todo_write", arguments: { op: "view" } },
				{ type: "toolCall", id: "todo-2", name: "todo_write", arguments: { op: "view" } },
			],
	...
	it("hands the trailing todo snapshot to the controller during mid-turn rebuild", () => {
		...
		const assistant = {
			role: "assistant",
			content: [{ type: "toolCall", id: "todo-1", name: "todo_write", arguments: { op: "view" } }],
			api: "anthropic-messages",
			provider: "anthropic",
			model: "claude-sonnet-4-5",
			stopReason: "stop",
