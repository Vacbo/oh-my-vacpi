...
describe("ACP agent", () => {
	...
	it("replays legacy todo_write tool results as ACP plan updates", async () => {
		const harness = await createHarness();
		const stored = new FakeAgentSession(harness.cwdA);
		harness.sessions.push(stored);
		// `todo_write` is the pre-rename spelling still present in stored
		// transcripts: replay must normalize it to the canonical `todo` so the
		// plan update survives. Live events use `todo` (see the test above).
		stored.sessionManager.appendMessage({
			role: "toolResult",
