...
const toolsWithComputerDisabled = await createTools(toolSession(Settings.isolated()), ["computer"]);
...
process.stdout.write(
	JSON.stringify({
		counts,
		disabledComputerToolCount: toolsWithComputerDisabled.filter(tool => tool.name === "computer").length,
		schema: {
