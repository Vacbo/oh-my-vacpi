...
describe("InteractiveMode plan.defaultOnStartup", () => {
	 ...
	 *  `extraRegistryTools` registers additional tools that are registered but
	 *  NOT in the initial active set, so a mode has to force-activate them on
	 *  entry. `builtInToolNames` marks which registry entries still have built-in
	...
	it("activates write when entering plan mode even if it was not initially active (issue #3165)", async () => {
		...
		// `write` and refine it with `edit`. `write` here sits in the registry
		// without being in the initial active set, so plan-mode entry must
		// force-activate it or the agent only has `edit`, which fails on a
		// non-existent file.
