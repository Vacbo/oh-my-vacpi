...
- Capped the fork's adaptive-thinking `max_tokens` raise at the Claude Code 64k output cap (`CLAUDE_CODE_MAX_OUTPUT_TOKENS`) adopted upstream in 15.10.4, so adaptive turns keep full headroom without exceeding the fingerprint-parity ceiling; the `/3` no-explicit-maxTokens default is likewise clamped through the same `Math.min`.
