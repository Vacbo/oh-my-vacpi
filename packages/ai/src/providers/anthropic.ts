...
function ensureMaxTokensForThinking(
	...
	if (thinking.type === "adaptive") {
		...
		const adaptiveCeiling = Math.min(model.maxTokens, CLAUDE_CODE_MAX_OUTPUT_TOKENS);
		if (adaptiveCeiling > 0 && (params.max_tokens ?? 0) < adaptiveCeiling) {
			params.max_tokens = adaptiveCeiling;
		}
		return;
	}
...
function buildParams(
	...
	const params: MessageCreateParamsStreaming = {
		...
		// ceiling (ensureMaxTokensForThinking raises adaptive turns back up), still clamped to
		// the Claude Code 64k output cap upstream enforces for fingerprint parity.
		max_tokens: Math.min(
			CLAUDE_CODE_MAX_OUTPUT_TOKENS,
			model.maxTokens,
			options?.maxTokens || (model.maxTokens / 3) | 0,
		),
		...(thinking && { thinking }),
