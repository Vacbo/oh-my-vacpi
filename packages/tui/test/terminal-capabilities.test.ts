...
describe("shouldEnableSynchronizedOutputByDefault", () => {
	...
	it("enables synchronized output by default on Warp (supports DEC 2026)", () => {
		expect(shouldEnableSynchronizedOutputByDefault({ TERM_PROGRAM: "WarpTerminal" }, "base")).toBe(true);
		// A multiplexer wrapping Warp still wins (screen-coordinate batching is unsafe through it).
		expect(shouldEnableSynchronizedOutputByDefault({ TERM_PROGRAM: "WarpTerminal", TMUX: "/tmp/x" }, "base")).toBe(
			false,
		);
		// The explicit kill switch still wins.
		expect(
			shouldEnableSynchronizedOutputByDefault({ TERM_PROGRAM: "WarpTerminal", PI_NO_SYNC_OUTPUT: "1" }, "base"),
		).toBe(false);
	});
});
