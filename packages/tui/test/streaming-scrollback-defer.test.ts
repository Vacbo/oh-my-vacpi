...
// (TERM_PROGRAM), or PI_TUI_RESIZE_IN_PLACE=1 steers a resize/commit onto the
// in-place path that skips the ED3 scrollback rebuild. Direct-terminal
// assertions clear all of them; the multiplexer-fallback describe re-sets TMUX
// afterward. The suite runs inside cmux (CMUX_WORKSPACE_ID/CMUX_SURFACE_ID),
// tmux/screen/zellij, or Warp on dev machines and CI.
...
function saveTerminalEnv(): Record<string, string | undefined> {
	// Neutralize multiplexer/terminal identity so the direct-terminal scrollback
	// assertions stay deterministic on any host, including cmux (a multiplexer or
	// Warp takes the in-place path that skips ED3).
