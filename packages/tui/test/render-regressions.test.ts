...
describe("TUI terminal-state regressions", () => {
	...
	beforeEach(() => {
		monotonicNow = 0;
		// Resize classification depends on multiplexer/terminal identity (a
		// multiplexer or Warp takes the in-place path that skips ED3), so
		// neutralize the ambient environment to keep these direct-terminal
		// assertions deterministic on any host, including cmux.
...
describe("foreground-tool streaming on ED3-risk terminals", () => {
	let savedTerminalEnv: Record<string, string | undefined> = {};
	beforeEach(() => {
		// Neutralize multiplexer/terminal identity so the clear-scrollback (ED3)
		// replay assertion below is deterministic on any host, including cmux.
		for (const key of DIRECT_TERMINAL_ENV_KEYS) {
			savedTerminalEnv[key] = Bun.env[key];
			delete Bun.env[key];
		}
