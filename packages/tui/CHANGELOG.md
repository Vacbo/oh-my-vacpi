...
- Added `encodeKey(keyId)`, the inverse of `matchesKey` for legacy (non-kitty) terminals: encodes named keys, printable characters, and `ctrl+`/`shift+`/`alt+`/`super+` combinations into the byte sequences a real terminal would send, falling back to the xterm modifyOtherKeys form where the raw control byte is claimed by a named key (e.g. `ctrl+m` vs Enter). Combos with no representation the matcher accepts in legacy mode (modified escape, modified f-keys) throw. Every supported id is round-trip-tested against the native matcher (`matchesKey(encodeKey(id), id)`).
...
- Added slash-command usage ranking so frequently used commands win equal text-match scores without outranking stronger prefix matches.
