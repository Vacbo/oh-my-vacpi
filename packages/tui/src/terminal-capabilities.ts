...
export function shouldEnableSynchronizedOutputByDefault(
	...
	// Warp supports DEC 2026 (Mode::SyncOutput) but is not on the terminalId allow-list,
	// and the runtime DECRQM probe only ever disables sync, never enables it, so opt Warp
	// in here (matched on TERM_PROGRAM). If a build ever lacks 2026, the probe turns it back off.
	if (env.TERM_PROGRAM?.toLowerCase() === "warpterminal") return true;
